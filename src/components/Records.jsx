...
export default function Records(props) {
  ...
  const fetchData = async () => {
    try {
      const response = props.admin?await fetch(`http://localhost:3000/fetch/workexperience/experience_type/${props.category}`):await fetch(`http://localhost:3000/fetch/workexperience/experience_type,faculty_id/${props.category+","+props.faculty_id}`);
  ...
  return (
    <div className="container mx-auto my-8 relative">
      ...
      <table className="min-w-full bg-white border border-gray-300">
        <thead>
          <tr>
            {props.admin&&<th className="py-2 px-4 border-b">Faculty_id</th>}
        ...
        <tbody>
          {showData.map((d, index) => (
            <tr key={index}>
              {props.admin&&<td className="py-2 px-4 border-b text-center">{d.faculty_id}</td>}
